...
```bash
ccd env create test-basic --type container
...
```bash
ccd env create test-fedora --type container --image fedora:latest
...
```bash
ccd env create test-ports --type container --image fedora:latest --port 8080:8080 --port 9090:9090
...
# Filter by type
ccd env list --type container
# Expected: only container environments

# JSON output
ccd env list -o json | jq '.[].name'

# Detailed status
ccd env status test-fedora
...
ccd env create test-orphan --type container --image fedora:latest
...
ccd env delete test-orphan --force
```
...
ccd env create test-creds --type container --image fedora:latest \
...
ccd env create test-autodetect --type container --image fedora:latest
...
ccd env exec test-fedora -- ls /workspace
# Expected: list of files in /workspace

ccd env exec test-fedora -- cat /etc/os-release
# Expected: Fedora release info

# Exec on stopped container
ccd env stop test-fedora
ccd env exec test-fedora -- ls
# Expected: error "container is not running"
ccd env start test-fedora
```
...
```bash
ccd env delete test-basic --force
...
```bash
ccd env delete test-fedora --force --keep-volumes
...
ccd env create test-config --type container
...
```bash
PATH=/usr/bin ccd env create test-nopodman --type container
...
```bash
ccd env create "INVALID NAME" --type container

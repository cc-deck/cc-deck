...
Add a `network` section to your `cc-deck-image.yaml` manifest, generate compose files, and deploy.
...
Blocked requests are logged and can be inspected with `cc-deck config domains blocked`.
...
[source,bash]
----
cc-deck config domains blocked my-session --since 4h
----

The command reads the proxy sidecar logs from the `my-session-proxy` container and filters for denied connection entries.

TIP: If a legitimate domain is being blocked, add it with `cc-deck config domains add` (see below) or include it in your `domains.yaml` configuration.
...
[source,bash]
----
cc-deck config domains add my-session registry.example.com
----
...
[source,bash]
----
cc-deck config domains remove my-session registry.example.com
----

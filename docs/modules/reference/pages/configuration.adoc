...
The optional `sharing.provider` setting selects the exposure provider used by workspace sharing commands such as `cc-deck ws start NAME --share`.
Its default is `cloudflare`, the only production provider included in V1.
...
The `--provider` flag overrides this value for one sharing operation.
Unknown or unavailable providers fail prerequisite validation before an invitation is issued.

Sharing runtime state is not configuration.
cc-deck stores non-secret operation metadata under `$XDG_STATE_HOME/cc-deck/` with restricted permissions so later lifecycle commands can reconcile an unclean exit.
Raw interactive and observer credentials are never persisted.
...
| `XDG_STATE_HOME`
| Base directory for runtime state. Default: `~/.local/state`
...
| `~/.local/state/cc-deck/`
| Restricted session-sharing operation state. Contains no raw invitation credentials.

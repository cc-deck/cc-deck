...
cc-deck ws voice mydev --mode ptt
...
| `--mode`
| Audio gating mode: `vad` (voice activity detection, default) or `ptt` (push-to-talk via F8)

| `--model`
| Whisper model name: `tiny.en`, `base.en` (default), `small.en`, `medium`

| `--device`
| Audio input device name (use `--list-devices` to enumerate)

| `--verbose`
| Enable detailed logging to `/tmp/cc-deck-voice.log`

...
For openshell targets, after a successful build the image is stamped with a `dev.cc-deck.policy-layer` label that records the diff ID of the layer containing `/etc/openshell/policy.yaml`.
This label enables fast policy extraction at runtime (see <<ws new>>).
If the policy file is not found in the built image, a warning is logged but the build does not fail.

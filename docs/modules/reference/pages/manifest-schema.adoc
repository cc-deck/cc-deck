...
The `cc-deck-image.yaml` manifest defines the desired contents of a container image.
...
| `endpoint`
| string
| Network endpoint in `host:port` format for OpenShell policy generation.
When set, a network policy entry is created allowing Claude Code to reach this MCP server.
Entries without an endpoint are skipped during policy assembly.
The capture command extracts this automatically from HTTP/SSE server URLs and stdio servers using `mcp-remote`.
|===

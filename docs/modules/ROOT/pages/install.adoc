...
After installing the CLI binary, run `cc-deck config plugin install` to set up the Zellij sidebar plugin and Claude Code hooks.
...
`make install` builds the WASM plugin, compiles the Go CLI, and runs `cc-deck config plugin install` to set up Zellij layouts and Claude Code hooks.
...
The `cc-deck config plugin install` command places files in your Zellij configuration directory:

...
fn perform_attend_directed(
    ...
    let tiers: Vec<Vec<AttendCandidate>> = {
        ...
        let mut done: Vec<_> = sessions
            ...
            .copied()
            .collect();
        done.sort_by_key(|s| std::cmp::Reverse(s.last_event_ts));
...
fn perform_working_directed(
    ...
    let tiers: Vec<Vec<AttendCandidate>> = {
        ...
        let mut working: Vec<_> = sessions
            ...
            .copied()
            .collect();
        working.sort_by_key(|s| std::cmp::Reverse(s.last_event_ts));
...
mod tests {
    ...
    fn state_with_sortable_sessions() -> ControllerState {
        let mut state = ControllerState::default();
    ...
    #[test]
    fn test_handle_sort_noop_with_fewer_than_two_sessions() {
        let mut state = ControllerState::default();
        let mut s1 = Session::new(1, "a".into());
        s1.activity = Activity::Working;
        s1.tab_index = Some(0);
    ...
    #[test]
    fn test_handle_sort_does_not_modify_tab_indices() {
        let mut state = ControllerState::default();
        let mut s0 = Session::new(10, "idle".into());
        s0.activity = Activity::Idle;
        s0.tab_index = Some(0);
        let mut s1 = Session::new(20, "work".into());
        s1.activity = Activity::Working;
        s1.tab_index = Some(1);

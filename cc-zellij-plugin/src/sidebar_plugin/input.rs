...
use super::modes::{FilterState, NavigateContext, NavigationOverlay, RenameState, SidebarMode};
...
pub fn handle_key(state: &mut SidebarState, key: KeyWithModifier) -> bool {
    ...
    match &state.mode {
        ...
        SidebarMode::Navigate {
            ...
            ..
        } => handle_navigate_key(state, key),
        SidebarMode::Navigate {
            overlay: NavigationOverlay::Filter(_),
            ..
        } => handle_filter_key(state, key),
        SidebarMode::Navigate {
            overlay: NavigationOverlay::DeleteConfirm(_),
            ..
        } => handle_delete_confirm_key(state, key),
        SidebarMode::Navigate {
            overlay: NavigationOverlay::Rename(_),
            ..
        } => handle_rename_key(state, key),
        SidebarMode::Navigate {
    ...
    match &state.mode {
        SidebarMode::Passive => {
            ...
            let ctx = NavigateContext {
                cursor_pane_id: pane_at_index(state, start),
            ...
            focus_self_wasm();
            crate::debug_log(&format!(
                "SIDEBAR NAV-PREV: entered Navigate cursor={start}"
            ));
        }
        SidebarMode::Navigate {
            overlay: NavigationOverlay::None,
...
fn pane_at_index(state: &SidebarState, index: usize) -> Option<u32> {
    state
        .filtered_sessions()
        .get(index)
...
fn move_cursor(state: &mut SidebarState, forward: bool) {
    ...
    let next = if sessions.is_empty() {
        ...
        Some(sessions[index].pane_id)
    };
    if let Some(ctx) = state.mode.nav_ctx_mut() {
        ctx.cursor_pane_id = next;
    }
}
...
fn handle_left_click(state: &mut SidebarState, row: usize) -> bool {
    ...
    if let Some(&(_r, pane_id, _)) = state.click_regions.iter().find(|(r, _, _)| *r == row) {
        if pane_id == super::render::VOICE_CLICK_SENTINEL {
            ...
            let current = state
                .local_mute_override
                .unwrap_or_else(|| state.cached_payload.as_ref().is_some_and(|p| p.voice_muted));
        ...
        if pane_id == super::render::SEPARATOR_CLICK_SENTINEL {
            ...
            if let Some(first) = sessions.first() {
                ...
                drop(sessions);
                switch_focus_locally(state, first_pane, first_tab);
            }
            return true;
        }
    ...
    if let Some((_r, pane_id, tab_index)) = hit {
        ...
        // Single click: switch focus locally (per-client) and report to controller
        switch_focus_locally(state, pane_id, tab_index);
    ...
    match key.bare_key {
        ...
        BareKey::Char('/') => {
            ...
            if let SidebarMode::Navigate { ctx, .. } = std::mem::take(&mut state.mode) {
                state.mode = SidebarMode::Navigate {
                    ctx,
                    overlay: NavigationOverlay::Filter(FilterState::default()),
                };
            }
            true
        }
...
fn handle_rename_key(state: &mut SidebarState, key: KeyWithModifier) -> bool {
    ...
    match action {
        ...
        RenameAction::Confirm(name) => {
            send_action(state, ActionType::Rename, Some(pane_id), None, Some(name));
...
fn exit_navigate(state: &mut SidebarState) {
    ...
    if let Some(ctx) = state.mode.nav_ctx() {
        ...
        if let (Some(pid), Some(tab)) = (ctx.restore_pane_id, ctx.restore_tab_index) {
            ...
            switch_focus_locally(state, pid, tab);
        }
    }
    state.mode = SidebarMode::Passive;
    state.filter_text.clear();
...
fn send_focus_report(state: &SidebarState, pane_id: u32, tab_index: usize) {
    let report = cc_deck::FocusReport {
        client_id: state.my_client_id,
        pane_id,
        tab_index,
    };
    send_focus_report_wasm(&report);
    crate::debug_log(&format!(
        "SIDEBAR FOCUS-REPORT: client={} pane={} tab={}",
        state.my_client_id, pane_id, tab_index
    ));
}

#[cfg(target_family = "wasm")]
fn send_focus_report_wasm(report: &cc_deck::FocusReport) {
    if let Ok(json) = serde_json::to_string(report) {
        let mut pipe_msg = MessageToPlugin::new("cc-deck:focus-report");
        pipe_msg.message_payload = Some(json);
        pipe_message_to_plugin(pipe_msg);
    }
}
...
#[cfg(not(target_family = "wasm"))]
fn send_focus_report_wasm(_report: &cc_deck::FocusReport) {}

/// Switch focus locally (per-client) and report to controller for presence tracking.
fn switch_focus_locally(state: &mut SidebarState, pane_id: u32, tab_index: usize) {
    state.local_focus_override = Some(pane_id);
    switch_tab_to_wasm(tab_index);
    focus_terminal_pane_wasm(pane_id);
    send_focus_report(state, pane_id, tab_index);
}
...
mod tests {
    ...
    fn test_help_any_key_dismisses() {
        ...
        assert!(matches!(state.mode, SidebarMode::Navigate { .. }));
    }
    ...
    fn test_nav_d_enters_delete_confirm() {
        ...
        match &state.mode {
            ...
            other => panic!(
                "expected NavigateDeleteConfirm, got {:?}",
    ...
    fn test_delete_y_confirms() {
        ...
        state.mode = SidebarMode::Navigate {
            ctx,
            overlay: NavigationOverlay::DeleteConfirm(10),
        };
        assert!(handle_delete_confirm_key(
            &mut state,
            bare(BareKey::Char('y'))
        ));
        assert!(matches!(state.mode, SidebarMode::Navigate { .. }));

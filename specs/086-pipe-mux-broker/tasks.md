...
- [ ] T005 [US1] Create `internal/mux/broker.go` with `Broker` struct: Unix socket listener via `net.ListenUnix` (SOCK_STREAM), accept loop goroutine reading newline-delimited JSON, dedup map (`map[string]Message` with mutex), flush timer (200ms ticker iterating map and calling `zellij pipe --session <session_name> --name <pipe_name> -- <args>` sequentially), atomic map swap on flush (replace with empty map, iterate old)
...
- [ ] T009 [P] [US1] Create `internal/mux/broker_test.go` with unit tests: broker starts and accepts connections, dedup collapses identical messages, flush delivers all unique messages, queue_size limit drops oldest, map swap is atomic (no lost messages during concurrent write+flush)
...
- [ ] T011 [US2] Modify `internal/cmd/hook.go` at the pipe delivery point (around line 151): before calling `exec.CommandContext(ctx, zellijPath, "pipe", ...)`, check `config.Mux.Enabled` and `$ZELLIJ_SESSION_NAME`. If both set, call `mux.SendOrStart(socketPath, sessionName, pipeName, payload)`. If it returns error, fall through to existing direct `zellij pipe` call

...
**Independent Test**: Can be fully tested by saving state with `cc-deck snapshot save`, restarting Zellij, and running `cc-deck snapshot restore`. Delivers immediate value by reducing multi-tab recovery from minutes of manual work to a single command.

**Acceptance Scenarios**:

1. **Given** a Zellij session with multiple tabs and Claude sessions, **When** the user runs `cc-deck snapshot save`, **Then** a state file is written containing all tab names, working directories, session IDs, display names, and pause flags in tab order.
2. **Given** a saved state file and a freshly started Zellij session, **When** the user runs `cc-deck snapshot restore`, **Then** new tabs are created for each saved session with the correct working directory, and Claude is started with `--resume` using the saved session ID.
...
1. **Given** an active cc-deck session with hook events firing, **When** a hook event is processed, **Then** the current session state is auto-saved to disk as a single `auto` snapshot (overwriting the previous one).
2. **Given** both the `auto` snapshot and named saves exist, **When** the user runs `cc-deck snapshot restore` without arguments, **Then** the most recent snapshot (auto or named, whichever is newest) is used.
...
**Independent Test**: Can be tested by saving with `cc-deck snapshot save ai-projects`, listing with `cc-deck snapshot list`, and restoring with `cc-deck snapshot restore ai-projects`.

**Acceptance Scenarios**:

1. **Given** an active session, **When** the user runs `cc-deck snapshot save my-setup`, **Then** state is saved with the name "my-setup" and persists indefinitely (not subject to auto-save rotation).
2. **Given** saved snapshots exist, **When** the user runs `cc-deck snapshot list`, **Then** all snapshots are listed with name, timestamp, and session count.
3. **Given** a named snapshot exists, **When** the user runs `cc-deck snapshot restore my-setup`, **Then** that specific snapshot is restored.
...
**Independent Test**: Can be tested by creating named saves, removing individual ones with `cc-deck snapshot remove <name>`, and clearing all with `cc-deck snapshot remove --all`.

**Acceptance Scenarios**:

1. **Given** a named snapshot "old-setup" exists, **When** the user runs `cc-deck snapshot remove old-setup`, **Then** that snapshot is deleted and no longer appears in `list`.
2. **Given** multiple snapshots exist, **When** the user runs `cc-deck snapshot remove --all`, **Then** all snapshots (both auto and named) are deleted.
3. **Given** no snapshot with the requested name exists, **When** the user runs `cc-deck snapshot remove nonexistent`, **Then** an error message is shown listing available snapshots.
...
- **FR-001**: System MUST provide a `cc-deck snapshot save [name]` command that captures current session state to a persistent file on the host filesystem. When no name is provided, a sequential name MUST be generated (`snapshot-1`, `snapshot-2`, etc., finding the next unused number). If a snapshot with the same name already exists, it MUST be silently overwritten.
- **FR-002**: System MUST provide a `cc-deck snapshot restore [name]` command that recreates tabs and starts Claude sessions from a saved state file, running inside an active Zellij session.
- **FR-003**: System MUST provide a `cc-deck snapshot list` command that displays all saved snapshots with name, timestamp, and session count.
- **FR-004**: System MUST provide a `cc-deck snapshot remove <name>` command that deletes a specific snapshot, and `cc-deck snapshot remove --all` to delete all snapshots.
...
- **FR-006**: The restore command MUST create new tabs in tab order, resolve the git root of the working directory (falling back to the directory itself for non-git directories), set that as the working directory, and start Claude with `--resume <session_id>`, falling back to a fresh Claude start if resume fails. Git root resolution is necessary because Claude Code stores sessions under the project root, not the subdirectory reported by hooks.
- **FR-007**: The restore command MUST show progress output for each tab being created (e.g., `Creating tab 1/5: cc-deck...`).
- **FR-008**: The hook command (`cc-deck hook`) MUST trigger auto-save by spawning a detached background process (`cc-deck snapshot save --auto`), overwriting a single `auto` snapshot each time. Auto-save MUST skip if the last auto-save was less than 5 minutes ago (cooldown). The background process MUST use a file lock (`flock`) to prevent concurrent auto-saves, and MUST use a context timeout (5s) for the plugin state query to avoid blocking indefinitely.
...
- **FR-010**: `cc-deck snapshot restore` without arguments MUST select the most recent snapshot (auto or named, whichever is newest by timestamp).
- **FR-011**: Named saves MUST persist indefinitely. The `auto` snapshot is always overwritten on the next auto-save.

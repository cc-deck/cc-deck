...
- **FR-002**: `cc-deck env create` MUST scaffold `.cc-deck/environment.yaml` and `.cc-deck/.gitignore` from CLI flags when no definition exists in a git repository. There is no separate init command; create handles both scaffolding and provisioning.
...
- **FR-025**: When `cc-deck env create` is run in a git repository with no `.cc-deck/environment.yaml`, the system MUST scaffold the definition from CLI flags before provisioning. When run outside a git repository with no definition, the system MUST require an explicit environment name.

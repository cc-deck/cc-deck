...
- The AUTH column in `ws ls -v` is derived at display time, not stored
...
1. **Given** a host with `ANTHROPIC_API_KEY` and `OPENAI_API_KEY` set, **When** the user creates a workspace, **Then** both credentials are injected without prompting (no conflict between different agents' credentials).
2. **Given** a host with no credentials for any auth mode, **When** the user creates a workspace, **Then** the workspace is created with no credentials injected (warning, not error).
3. **Given** a host with Vertex credentials only, **When** the user creates a workspace, **Then** vertex credentials are auto-injected (single available Claude mode).
...
### User Story 7 - Generalized SSH credential handling (Priority: P2)

SSH credential resolution uses agent-declared specs instead of hardcoded Claude-only detection. The `detectAuthMode()` function in SSH is replaced by the detect-all model.

**Why this priority**: Removes the Claude-only hardcoding that blocks multi-agent support in SSH workspaces.
...
- What happens when two auth modes from the SAME agent are both available?
  All detected modes are injected. If two modes set the same env var to different values, `MergeCredentials` returns an error. Future work may add interactive conflict resolution and opt-out.
...
- **SC-006**: The hardcoded `detectAuthMode()` function in SSH credentials is replaced by agent-declared specs (with legacy fallback preserved for backwards compatibility).
...
- Same-agent conflict resolution (interactive prompting, opt-out lists, `--exclude`/`--include` flags) is deferred to a future iteration. Currently, if two modes from the same agent set the same env var to different values, `MergeCredentials` returns an error.
- OpenShell credential refactoring to use agent-declared specs is deferred to a future iteration. The existing `KnownProviderProfiles` map remains active for OpenShell workspaces.

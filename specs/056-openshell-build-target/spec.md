...
- **FR-003b**: The generated Containerfile MUST include the full cc-deck stack (Zellij, cc-deck plugin, cc-session, cc-setup, Claude Code) adapted for the `sandbox` user and `/sandbox` paths. The user experience inside an OpenShell sandbox MUST match the container target (same TUI, same tools, same hooks).

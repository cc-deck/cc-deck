...
Parked: This requires the pluggable tunnel interface (083) to be designed first, and findings from the spike (082) about how Zellij's web server behaves in different environments.

**Preliminary direction:** Start with local + SSH (both can use localhost tunneling via port forwards). OpenShell and K8s backends can expose sessions through their native networking (K8s Ingress, OpenShell gateway).

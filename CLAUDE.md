...
- Go 1.25 (from go.mod) + cobra v1.10.2 (CLI), bubbletea v0.25+ (TUI), lipgloss v1.x (styling), bubbles (components), internal/env (environment management) (031-session-tui)
- `$XDG_STATE_HOME/cc-deck/state.yaml` (existing), `~/.config/zellij/plugins/cc_deck.wasm/cache/sessions.json` (read-only) (031-session-tui)
...
## Recent Changes
- 031-session-tui: Added Go 1.25 (from go.mod) + cobra v1.10.2 (CLI), bubbletea v0.25+ (TUI), lipgloss v1.x (styling), bubbles (components), internal/env (environment management)

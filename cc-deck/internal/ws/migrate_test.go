...
func TestMigrateFromConfig_WithSessions(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, WorkspaceTypeK8sDeploy, inst1.Type)
	assert.Equal(t, WorkspaceStateRunning, inst1.State)
	...
	require.NoError(t, err)
	assert.Equal(t, WorkspaceStateUnknown, inst2.State)
...
func TestMigrateFromConfig_Idempotent(t *testing.T) {
	...
	list, err := store.ListInstances(nil)
	require.NoError(t, err)
	assert.Len(t, list, 1, "should not duplicate workspace on re-migration")
}
...
func TestMapSessionStatus(t *testing.T) {
	tests := []struct {
		...
		{"running", WorkspaceStateRunning},
		{"Running", WorkspaceStateRunning},
		{"RUNNING", WorkspaceStateRunning},
		{"stopped", WorkspaceStateUnknown},
		{"error", WorkspaceStateUnknown},

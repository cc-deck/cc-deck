...
func TestComposeWorkspace_Type(t *testing.T) {
	e := &ComposeWorkspace{name: "test"}
	assert.Equal(t, WorkspaceTypeCompose, e.Type())
}

func TestComposeWorkspace_Name(t *testing.T) {
	e := &ComposeWorkspace{name: "myenv"}
	assert.Equal(t, "myenv", e.Name())
}

func TestComposeWorkspace_SessionContainerName(t *testing.T) {
	e := &ComposeWorkspace{name: "myenv"}
	assert.Equal(t, "cc-deck-myenv", e.sessionContainerName())
}

func TestComposeWorkspace_ProxyContainerName(t *testing.T) {
	e := &ComposeWorkspace{name: "myenv"}
	assert.Equal(t, "cc-deck-myenv-proxy", e.proxyContainerName())
}

func TestComposeWorkspace_ProjectDir_Default(t *testing.T) {
	e := &ComposeWorkspace{name: "test"}
	dir := e.projectDir()
	cwd, _ := os.Getwd()
...
func TestComposeWorkspace_ProjectDir_Explicit(t *testing.T) {
	e := &ComposeWorkspace{name: "test", ProjectDir: "/tmp/myproject"}
	assert.Equal(t, "/tmp/myproject", e.projectDir())
}

func TestComposeWorkspace_ComposeProjectDir(t *testing.T) {
	e := &ComposeWorkspace{name: "test", ProjectDir: "/tmp/myproject"}

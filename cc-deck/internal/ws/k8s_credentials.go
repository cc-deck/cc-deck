...
func HandleCredentials(ctx context.Context, client *K8sClient, wsName, ns string, labels, creds map[string]string, existingSecret, secretStore, secretStoreRef, secretPath string) (*CredentialResult, error) {
	result := &CredentialResult{}

	// Inline credentials: create a K8s Secret.
	if len(creds) > 0 {
		secret := generateCredentialSecret(wsName, ns, labels, creds)
	...
	if secretStore != "" {
		...
		eso := generateExternalSecret(wsName, ns, labels, secretStoreRef, secretPath)
...
func generateCredentialSecret(wsName, ns string, labels map[string]string, creds map[string]string) *corev1.Secret {
	...
	return &corev1.Secret{
		ObjectMeta: metav1.ObjectMeta{
			Name:      k8sResourceName(wsName) + "-creds",
			Namespace: ns,
			Labels:    labels,
		},
...
func generateExternalSecret(wsName, ns string, labels map[string]string, secretStoreRef, secretPath string) *unstructured.Unstructured {
	resName := k8sResourceName(wsName) + "-eso"

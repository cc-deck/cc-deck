...
func TestLoad_ValidFile(t *testing.T) {
	...
	original := &StateFile{
		...
		Instances: []WorkspaceInstance{
			{Name: "test-env", Type: WorkspaceTypeLocal, State: WorkspaceStateRunning},
		},
	}
...
func TestAllProjectWorkspaceNames(t *testing.T) {
	store := newTestStore(t)
	...
	require.NoError(t, SaveProjectDefinition(projA, &WorkspaceDefinition{Name: "alpha", Type: WorkspaceTypeCompose}))
	require.NoError(t, SaveProjectDefinition(projB, &WorkspaceDefinition{Name: "beta", Type: WorkspaceTypeCompose}))
	require.NoError(t, SaveProjectDefinition(projC, &WorkspaceDefinition{Name: "gamma", Type: WorkspaceTypeCompose}))
	...
	t.Run("all projects", func(t *testing.T) {
		names, err := store.AllProjectWorkspaceNames("")
		require.NoError(t, err)
	...
	t.Run("exclude one project", func(t *testing.T) {
		names, err := store.AllProjectWorkspaceNames(projB)
		require.NoError(t, err)

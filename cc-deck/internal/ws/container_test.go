...
func TestSecretName(t *testing.T) {
	...
	for _, tt := range tests {
		assert.Equal(t, tt.want, secretName(tt.wsName, tt.key))
	}
}
...
func TestContainerWorkspace_Type(t *testing.T) {
	store := newTestStore(t)
	env := &ContainerWorkspace{name: "test", store: store}
	assert.Equal(t, WorkspaceTypeContainer, env.Type())
}

func TestContainerWorkspace_Name(t *testing.T) {
	store := newTestStore(t)
	env := &ContainerWorkspace{name: "my-container", store: store}
	assert.Equal(t, "my-container", env.Name())
}

func TestContainerWorkspace_CreateRejectsInvalidName(t *testing.T) {
	store := newTestStore(t)
	env := &ContainerWorkspace{name: "INVALID", store: store}
...
func TestContainerWorkspace_CreateRequiresPodman(t *testing.T) {
	if !podman.Available() {
	...
	store := newTestStore(t)
	env := &ContainerWorkspace{name: "valid-name", store: store}
...
func TestContainerWorkspace_DeleteBestEffort(t *testing.T) {
	...
	store := newTestStore(t)
	env := &ContainerWorkspace{name: "nonexistent-env", store: store}

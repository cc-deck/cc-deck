...
- `--fresh`: Start from scratch. Deletes `build.yaml` and the `config/` directory in the setup directory before running the wizard. Existing cached catalog components (`openshell/components/`) and snippets are preserved since they are regenerated by `build refresh`.

Parse `$ARGUMENTS` for flags before proceeding. If `--all` is present, set `AUTO_ACCEPT = true`. If `--fresh` is present, set `FRESH_START = true`. Both flags can be combined (`--all --fresh`).

**Fresh start cleanup** (if `FRESH_START` is true):

```bash
SETUP_DIR="<resolved setup directory>"
rm -f "$SETUP_DIR/build.yaml"
rm -rf "$SETUP_DIR/config/"
echo "Cleared manifest and config files. Starting fresh."
```

Then proceed with the wizard as if no manifest exists.

...
func InitSetupDir(dir string, projectRoot string, force bool, targets []string) error {
	...
	// Create config directory for shared settings
	...
	if hasSSH {
		if err := scaffoldSSHRoles(filepath.Join(dir, "ssh")); err != nil {
			return err
		}
	}
...
func uncommentTargets(content string, container, ssh, openshell bool) string {
	lines := strings.Split(content, "\n")
	var result []string
	inTargetsBlock := false
	inContainerSection := false
	inSSHSection := false
	inOpenShellSection := false
	...
	for i, line := range lines {
		...
		if inTargetsBlock {
			...
			if trimmed == "#   openshell:" {
				inOpenShellSection = true
				inContainerSection = false
				inSSHSection = false
				if openshell {
					result = append(result, "  openshell:")
				} else {
					result = append(result, line)
				}
				continue
			...
			if inOpenShellSection && openshell {
				uncommented := uncommentLine(line)
				result = append(result, uncommented)
				continue
			}
...
func scaffoldSSHRoles(dir string) error {
	for _, role := range sshRoles {
		...
		header := fmt.Sprintf("---\n# %s role tasks\n# Generated by cc-deck build init. Content populated by /cc-deck.build --target ssh.\n", role)

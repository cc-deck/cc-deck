...
// K8sClient wraps Kubernetes client-go for environment lifecycle operations.
...
// EnvironmentState for the given StatefulSet.
func (c *K8sClient) ReconcileState(ctx context.Context, ns, stsName string) (EnvironmentState, error) {
	sts, err := c.clientset.AppsV1().StatefulSets(ns).Get(ctx, stsName, metav1.GetOptions{})
	if err != nil {
		if errors.IsNotFound(err) {
			return EnvironmentStateError, nil
		}
		return EnvironmentStateUnknown, err
	}

	if sts.Spec.Replicas != nil && *sts.Spec.Replicas == 0 {
		return EnvironmentStateStopped, nil
	}

	if sts.Status.ReadyReplicas > 0 {
		return EnvironmentStateRunning, nil
	}

	return EnvironmentStateCreating, nil
}

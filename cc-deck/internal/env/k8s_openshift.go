...
func GenerateRoute(envName, ns, serviceName string, labels map[string]string) *unstructured.Unstructured {
...
func GenerateEgressFirewall(envName, ns string, labels map[string]string, domains []string) *unstructured.Unstructured {
	resName := k8sResourceName(envName)
